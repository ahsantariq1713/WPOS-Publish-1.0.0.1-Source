...
{
    public partial class ItemListView : Form
    {
        ...
        public IEnumerable<Item> Entities { get;private set; }
        public ItemListView(Adapter adapter, Form owner = null)
        {
        ...
        {
            Entities = await Adapter.Retrive<Item>(withRelatedData: true);
        ...
        public void LoadComponents(IEnumerable<Item> entities)
        {

            grid.DataSource = typeof(IEnumerable<Item>);
            grid.DataSource = entities.ToList() as IEnumerable<Item>;
            ...
            var c0 = grid.Columns[nameof(Item.Code)];
            c0.Visible = true;
            c0.DisplayIndex = 0;
            c0.FillWeight = 50;

            var c1 = grid.Columns[nameof(Item.Name)];
            c1.Visible = true;
            c1.DisplayIndex = 1;
            c1.FillWeight = 200;

            var c2 = grid.Columns[nameof(Item.Brand)];
            c2.Visible = true;
            c2.DisplayIndex = 2;
            c2.FillWeight = 150;

            var c3 = grid.Columns[nameof(Item.Category)];
            c3.Visible = true;
            c3.DisplayIndex = 3;
            c3.FillWeight = 150;

            var c4 = grid.Columns[nameof(Item.Unit)];
            c4.Visible = true;
            c4.DisplayIndex = 4;
            c4.FillWeight = 100;

            var c5 = grid.Columns[nameof(Item.Notes)];
            c5.Visible = true;
            c5.DisplayIndex = 5;
            c5.FillWeight = 300;
        }
        ...
        private async void BtnEditRow_Click(object sender, EventArgs e)
        {
            if (grid.SelectedRows.Count > 0 && grid.SelectedRows[0].Index != -1)
            {
                ...
                await Adapter.Resolve<ItemEditView, int, Form>(entityId, this);
        ...
        private async void Button6_Click(object sender, EventArgs e)
        {
          ...
          Entities = (await Adapter.Retrive<Item>(withRelatedData:true,activeOnly: !chkdeleted.Checked))?.ToList()
                .FindAll(x =>
                {
        ...
        private void TxtItemSearch_Leave(object sender, EventArgs e)
        {
            ...
            {
                txtSearch.Text = "Search item by name, brand or category name";
        ...
        private async void BtnFindItem_Click(object sender, EventArgs e)
        {
            ...
            var entities =
                (await Adapter.Retrive<Item>(withRelatedData:true,activeOnly:!chkdeleted.Checked))?.ToList()
                .FindAll(x =>
                {
                   var search = XString.Equal(x.Brand, txtSearch.Text) || 
        ...
        private async void BtnDeleteRow_Click(object sender, EventArgs e)
        {
            if (grid.SelectedRows.Count > 0 && grid.SelectedRows[0].Index != -1)
            {
                ...
                var customers = grid.DataSource as List<Item>;
                var deleteable = new List<Item>();
                var changes = 0;
