...
namespace WirelessPOS
{
    public partial class SupplierListView : Form
    {
        ...
        public IEnumerable<Supplier> Entities { get;private set; }
        public SupplierListView(Adapter adapter, Form owner = null)
        {
        ...
        {
            Entities = await Adapter.Retrive<Supplier>(withRelatedData: true);
        ...
        public void LoadComponents(IEnumerable<Supplier> entities)
        {

            grid.DataSource = typeof(IEnumerable<Supplier>);
            grid.DataSource = entities.ToList() as IEnumerable<Supplier>;
            ...
            var c0 = grid.Columns[nameof(Supplier.Id)];
            c0.Visible = true;
            c0.DisplayIndex = 0;
            c0.FillWeight = 50;

            var c1 = grid.Columns[nameof(Supplier.Name)];
            c1.Visible = true;
            c1.DisplayIndex = 1;
            c1.FillWeight = 200;

            var c2 = grid.Columns[nameof(Supplier.Company)];
            c2.Visible = true;
            c2.DisplayIndex = 2;
            c2.FillWeight = 150;

            var c3 = grid.Columns[nameof(Supplier.Phone)];
            c3.Visible = true;
            c3.DisplayIndex = 3;
            c3.FillWeight = 150;

            var c4 = grid.Columns[nameof(Supplier.Email)];
            c4.Visible = true;
            c4.DisplayIndex = 4;
            c4.FillWeight = 150;

            var c5 = grid.Columns[nameof(Supplier.Address)];
            c5.Visible = true;
            c5.DisplayIndex = 5;
            c5.FillWeight = 300;
        ...
        private async void BtnEditRow_Click(object sender, EventArgs e)
        {
            if (grid.SelectedRows.Count > 0 && grid.SelectedRows[0].Index != -1)
            {
                ...
                await Adapter.Resolve<SupplierEditView, int, Form>(entityId, this);
        ...
        private async void Button6_Click(object sender, EventArgs e)
        {
            ...
            Entities =
                (await Adapter.Retrive<Supplier>(activeOnly: !chkdeleted.Checked))?.ToList()
                .FindAll(x =>
                {
        ...
        private async void Button5_Click(object sender, EventArgs e)
        {
            ...
            await Adapter.Resolve<SupplierEditView, int, Form>(0, this);
        ...
        private async void BtnFindItem_Click(object sender, EventArgs e)
        {
            ...
            var entities =
                ...
                .FindAll(x =>
                {
                    var search = XString.Equal(x.Phone.RemovePhoneFormat(), txtSearch.Text) || XString.Contains(x.Email, txtSearch.Text) || XString.Contains(x.Name, txtSearch.Text);
                    if (panelDates.Enabled == true)
                    {
        ...
        private async void BtnDeleteRow_Click(object sender, EventArgs e)
        {
            if (grid.SelectedRows.Count > 0 && grid.SelectedRows[0].Index != -1)
            {
                ...
                var customers = grid.DataSource as List<Supplier>;
                var deleteable = new List<Supplier>();
                var changes = 0;
