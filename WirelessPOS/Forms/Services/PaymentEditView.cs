...
{
    public partial class PaymentEditView : Form
    {
        private readonly Form owner;
        private WaitWin wait;
        private List<Carrier> carriers;
        private List<MonthlyPlan> mPlans;
        private List<Company> companies;

        public Payment Entity { get; private set; }
        public Adapter Adapter { get; }

        public PaymentEditView(Adapter adapter, int id, Form owner = null)
        {
            Adapter = adapter;
            this.owner = owner;
            //Owner = owner;
        ...
        {
            if (id == 0) Entity = new Payment() { Tax = Properties.Settings.Default.Tax };
            else
            {
                Entity = await Adapter.Retrive<Payment>(id);
                if (Entity == null)
                {
                    wait.Close();
                    Message.Stop("No Payment found to show.");
                    this.Dispose();
                    return;
                }
            ...
            carriers = (await Adapter.Retrive<Carrier>())?.ToList();
            BindCarrier();

            companies = (await Adapter.Retrive<Company>())?.ToList();
            BindCompany();

            mPlans = (await Adapter.Retrive<MonthlyPlan>())?.ToList();
            BindMonthlyPlans();
        ...
        {
            carrierBindingSource.Clear();
            carriers?.ToList().ForEach(x =>
            {
                carrierBindingSource.Add(x);
            });
        }

        private void DdlFromCarrier_TextChanged(object sender, EventArgs e)
        {
            var text = ddlCarrier.Text;
            if (text.IsNull())
            {
                btnAddCarrier.Enabled = false;
                return;
            }
            var found = carriers?.Find(x => XString.Equal(x.Name, text));
            if (found != null) btnAddCarrier.Enabled = false;
            else btnAddCarrier.Enabled = true;
        }

        private async void BtnAddCarrier_Click(object sender, EventArgs e)
        {
            if (btnAddCarrier.IsBusy()) return;
            btnAddCarrier.MarkBusy(out Image image, out Color color);
            var carrier = new Carrier { Name = ddlCarrier.Text };
            int changes = await Adapter.Save(carrier);
            btnAddCarrier.MarkFree(image, color);
            if (changes <= 0) return;
            carriers.Add(carrier);
            carrierBindingSource.Add(carrier);
            ddlCarrier.SelectedItem = carrier;
            btnAddCarrier.Enabled = false;
        }
        ...
        private void BindMonthlyPlans()
        {
            monthlyPlanBindingSource.Clear();
            mPlans?.ToList().ForEach(x =>
            {
                monthlyPlanBindingSource.Add(x);
            });
        }

        private void DdlMonthlyPlan_TextChanged(object sender, EventArgs e)
        {
            var text = ddlMonthlyPlan.Text;
            if (text.IsNull())
            {
                btnAddMonthlyPlan.Enabled = false;
                return;
            }
            var found = mPlans?.Find(x => XString.Equal(x.Name, text));
            if (found != null) btnAddMonthlyPlan.Enabled = false;
            else btnAddMonthlyPlan.Enabled = true;
        }

        private async void BtnAddMonthlyPlan_Click(object sender, EventArgs e)
        {
            if (btnAddMonthlyPlan.IsBusy()) return;
            btnAddMonthlyPlan.MarkBusy(out Image image, out Color color);
            var monthlyPlan = new MonthlyPlan { Name = ddlMonthlyPlan.Text };
            int changes = await Adapter.Save(monthlyPlan);
            btnAddMonthlyPlan.MarkFree(image, color);
            if (changes <= 0) return;
            mPlans.Add(monthlyPlan);
            monthlyPlanBindingSource.Add(monthlyPlan);
            ddlMonthlyPlan.SelectedItem = monthlyPlan;
            btnAddMonthlyPlan.Enabled = false;
        }
        ...
        public void BindEntity()
        {
            ...
            paymentBindingSource.Add(Entity);
        }
        ...
        private async void BtnFinish_Click(object sender, EventArgs e)
        {
            ...
            if (changes > 0)
            {
                if (Entity.Transactions?.Count == 0) new TransactionView<Payment>(Adapter, Entity, TransactionType.Cashin, this);
        ...
        {
            new TransactionView<Payment>(Adapter, Entity, TransactionType.Cashin, this);
            (Master.Instance.OperatorView as OperatorView)?.RefreshNotificationCenter();
            BindEntity();
            InvalidateForm();
        ...
        private void PrintReceipt(XPrinter printer)
        {
            ...
            MessageBox.Show("printing carrier...");
            printer.AddString(string.Format("{0,-" + W(15) + "}{1,-" + W(35) + "}{2,-" + W(15) + "}{3,-" + W(35) + "}", "Carrier", ": "+Entity.Carrier.Name, "Company", ": " + Entity.Company.Name), XFont.R8, XFontColor.Black, g: graphics);
            MessageBox.Show("printing plan...");
            printer.AddString(string.Format("{0,-" + W(15) + "}{1,-" + W(35) + "}{2,-" + W(15) + "}{3,-" + W(35) + ":C2}", "Monthly Plan", ": " + Entity.MonthlyPlan.Name, "Rate", ": " + Entity.Rate), XFont.R8, XFontColor.Black, g: graphics);
            printer.AddVerticalSpace(graphics);
            MessageBox.Show("printing promo...");
            ...
            MessageBox.Show("printing transactions...");
        ...
        {
            new TransactionView<Payment>(Adapter, Entity, TransactionType.Refund, this);
            (Master.Instance.OperatorView as OperatorView)?.RefreshNotificationCenter();
            BindEntity();
            InvalidateForm();
        }
